...
import {DELETE, DELETE_INCOME, GET_CATEGORIES_INCOME, INCOME, POST, PUT} from "../../../config/config";
...
export class IncomeService{
    ...
    static async getIncomes() {
        ...
        if (result.redirect || result.error || !result.response && (result.response && (result.response.error || !result.response))) {
            returnObject.error = 'Ошибка при запросе доходов';
            if (result.redirect) {
    ...
    static async getIncome(id) {
        ...
        if (result.redirect || result.error || !result.response && (result.response && (result.response.error || !result.response))) {
            returnObject.error = 'Ошибка при запросе дохода';
            if (result.redirect) {
                return returnObject.redirect = result.redirect;
            }
            return returnObject;
    ...
    static async createIncome(id, data) {
        const returnObject = {
            error: false,
            redirect: null,
            title: null,
        ...
        const result = await HttpUtils.request(GET_CATEGORIES_INCOME + '/' + id, POST, true, data);
